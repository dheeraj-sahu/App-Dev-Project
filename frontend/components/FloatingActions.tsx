...
export default function FloatingActions({ onScanPress }: FloatingActionsProps) {
  ...
  const isHome =
    pathname === "/" || pathname === "/index" || pathname === "/(tabs)/index"; // exact match might be needed depending on router setup
    ...
    switch (action) {
      case "scan":
        if (onScanPress) {
          ...
          router.push("/ocr?start=camera");
        }
        break;
      case "voice":
        Alert.alert("Coming Soon", "Voice commands are not yet implemented.");
        break;
      case "manual":
  ...
  return (
    <View style={styles.container}>
      {/* Backdrop to close when tapping outside - optional but good UX */}
      ...
      <Animated.View style={[styles.menuContainer, animatedMenuStyle]}>
        ...
        >
          <View style={[styles.actionIcon, { backgroundColor: "#E8F5E9" }]}>
            <Ionicons name="mic" size={20} color={theme.income} />
          </View>
          <Text style={[styles.actionLabel, { color: theme.text }]}>
            Voice Command
          </Text>
...
const styles = StyleSheet.create({
  ...
  backdrop: {
    ...
    left: -1000,
    right: 1000,
    bottom: -1000, // naive fullscreen coverage relative to this view
